...
export default function ProductDetail() {
  ...
  const fetchRatingStats = async () => {
    try {
      const { data } = await reviewService.getProductStats(id);
      setRatingStats(data);
    } catch (error) {
  ...
  const fetchUserReview = async () => {
    try {
      const { data } = await reviewService.getUserProductReview(id);
      if (data) {
        setUserReview(data);
        setReviewForm({
          rating: data.rating,
      ...
      // User has no review, that's fine
  ...
  const RatingStars = ({ rating, size = "1rem" }) => {
    return (
        ...
        {showReviewForm && user && (
          ...
          >
            ...
            <div style={{ marginBottom: "1rem" }}>
              <label style={{ display: "block", marginBottom: "0.5rem" }}>
                Rating: <RatingStars rating={parseInt(reviewForm.rating)} />
              </label>
              <select
